...
$i18n = [
    ...
    "edit_widgets_hint" => "ドラッグで並べ替え。目アイコンで表示/非表示。歯車で支払い方法別予算を設定、+でもう一つ追加。",
    "reorder_widget" => "Drag to reorder",
    "show_widget" => "Show widget",
    "hide_widget" => "Hide widget",
    "configure_payment_method_budget" => "支払い方法を設定",
    "remove_payment_method_budget" => "このウィジェットを削除",
    "add_payment_method_budget" => "支払い方法別予算を追加",
    "widget_title" => "ウィジェットタイトル",
    "select_payment_methods" => "支払い方法",
    "payment_method_budget_select_info" => "すべて未選択のままにすると、予算がある支払い方法をすべて含めます。またはこのウィジェット用に特定の方法を選びます。",
    "payment_method_budget_display_mode" => "表示",
    "payment_method_budget_display_per_method" => "支払い方法ごとに別行で表示",
    "payment_method_budget_display_combined" => "選択した方法を合計して1行にまとめる",
    "payment_method_budget_display_info" => "合計表示では、選択した支払い方法の予算と必要額を1行に合算します。",
    "confirm_remove_payment_method_budget" => "この支払い方法別予算ウィジェットを削除しますか？",
    "cannot_remove_last_payment_method_budget" => "支払い方法別予算ウィジェットを少なくとも1つ残すか、目アイコンで非表示にしてください。",
    "widget_no_data" => "Nothing to show yet",

    "budget_period" => "予算期間",
    "budget_anchor_date" => "基準日",
    "fortnightly" => "隔週",
    "cost_vs_monthly_budget" => "費用 対 月間予算",
    "cost_vs_period_budget" => "費用 対 期間予算",
    "send_period_summary_at_period_start" => "各支払い期間の開始時にまとめを送信",
    "amount_for_pay_period" => "支払い期間の金額",
    "remaining" => "残り",
    "amount_needed_this_period" => "今期に必要な金額",
    ...
    "push_notifications_info" => "専用のアカウントやアプリを用意することなく、ブラウザやスマートフォンのロック画面に通知を受け取れます。デスクトップのChrome、Firefox、Edge、Androidで動作し、iOSではWallosをアプリとしてホーム画面に追加した後に利用できます。",
    "no_devices_registered" => "まだ登録されたデバイスはありません。",
    "unknown_device" => "不明なデバイス",
    "enable_on_this_device" => "このデバイスで有効にする",
    "categories" => "カテゴリ",
    ...
    "coming_soon" => "近日公開",
    "invalid_host" => "無効なホスト",
    "ai_provider_error" => "AIプロバイダーのエラー",
    "ai_invalid_response" => "AIプロバイダーが無効な応答を返しました。",
    "ai_invalid_json_response" => "AIの応答に有効な推奨事項が含まれていませんでした。",
    ...
    "ssrf_allowlist_env_managed" => "SSRF_ALLOWLIST 環境変数によって管理されています。",
    "allow_standard_users_local_webhooks" => "標準ユーザーによる Webhook 許可リストの使用を許可する",
    "allow_standard_users_local_webhooks_info" => "セキュリティリスク: これを有効にすると、すべての標準ユーザー(管理者以外)が上記の Webhook 許可リストに記載されたアドレス(ネットワーク内の内部サービスを含む)に通知を送信できるようになります。この Wallos インスタンスのすべてのユーザーを信頼できる場合にのみ有効にしてください。",
